...
app.post('/telegram-webhook', async (req, res) => {
  ...
  try {
    ...
    if (update.message) {
      ...
      if (text === '/start') {
        broadcastEvent({ type: 'log', node: 'telegram', message: '텔레그램 /start 명령 수신' });
        await sendTelegram(
          '👋 안녕하세요! <b>자비스 알림봇</b>입니다.\n\n' +
          '매일 오전 9시에 스마트스토어 주문 현황을 자동으로 보고합니다.\n\n' +
          '사용 가능한 명령어:\n' +
          '• /report - 지금 즉시 주문 현황 조회\n' +
          '• /start - 봇 정보 확인'
        );
      } else if (text === '/report') {
        broadcastEvent({ type: 'node_active', node: 'brain', message: '텔레그램 /report 명령 수신 - 주문 조회 시작', progress: 0, flow: ['telegram->brain'] });
        await sendTelegram('🔄 주문 현황을 조회하고 있습니다...');
        await runDailyOrderReport();
